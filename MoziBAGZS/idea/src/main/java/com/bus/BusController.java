...
public class BusController {
    ...
    public String addMovie(@RequestParam String movieName,
                           @RequestParam String movieDetails,
    ...
    public String deleteMovie(@RequestParam String movieName, Model model, HttpSession session) {
        Customer user = (Customer) session.getAttribute("user");
        long bid = user.getBid();
        if (bid == 1) {
            MovieDetails movie = movieRepo.findByMovieName(movieName);
            if (movie != null) {
    ...
    public String login(@RequestParam("email") String email, @RequestParam("password") String password,
        ...
        if (object != null) {
            ...
            if (customer == null) {
                m.addAttribute("failed", "Sikertelen bejelentkezés!");
                return "login";
            } else {
...
//		System.out.println(message);
    ...
    public String getUser(@RequestParam("movieName") String movieName, HttpSession session, Model m) {
        // Lekéri az összes filmet az adatbázisból
        List<MovieDetails> movieList = dao.getAllMovie();

        // Keresés a movieName alapján
        MovieDetails selectedMovie = movieList.stream()
                .filter(movie -> movie.getMovieName().equals(movieName))
                .findFirst()
                .orElse(null);

        if (selectedMovie != null) {
            // Ha a film létezik, beállítja a session attribútumokat
            session.setAttribute("movieName", selectedMovie.getMovieName());
            session.setAttribute("movieDetails", selectedMovie.getMovieDetails());
            ...
            List<Seat> userSeats = customer.getSeat();

            // Lekéri az összes foglalt ülést az adott időpontra
            List<Seat> allSeats = dao.getAllSeat(now, time);
            for (Seat seat : allSeats) {
                bookedSeatNumbers.addAll(seat.getSeatNo());
            }

            // Hozzáadja a modell attribútumokat a nézethez
            m.addAttribute("date", now);
            m.addAttribute("time", time);
            m.addAttribute("max", monthLimit.plusMonths(1));
            m.addAttribute("min", monthLimit);
            m.addAttribute("seats", bookedSeatNumbers);
            m.addAttribute("seat", userSeats);
            m.addAttribute("movieDetails", selectedMovie.getMovieDetails());

            session.setAttribute("user", customer);
            return "dashboard";
        } else {
            // Ha a film nem található, visszairányít a kezdőlapra
            return "redirect:/home";
        }
    }
    ...
    public String bookSeat(@ModelAttribute("Seat") Seat seat, @RequestParam("movieName") String movieName,
        ...
        if (object == null) {
            ...
            if (((date.isAfter(currentDate)) || (date.equals(currentDate)))
                ...
                session.setAttribute("msg", "Sikeres foglalás!");
                return "redirect:/home";
